        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <nav className="flex items-center gap-6">
            ...
            {user ? (
              <>
                {isAdmin && (
                  <Link href="/admin" className="text-sm text-lime-400 hover:text-lime-300 transition-colors">
                    Admin
                  </Link>
                )}
              ...
              <>
                ...
                <Link
                  href="/signup"
                  className="text-sm px-4 py-2 bg-lime-600 hover:bg-lime-700 text-white rounded-lg transition-colors"
                >
                  Sign Up
                </Link>
              </>
      ...
      <section className="max-w-7xl mx-auto px-4 py-24">
        <div className="text-center space-y-6">
          <h1 className="text-5xl font-bold text-white leading-tight">
            Find the Best <span className="text-lime-400">Crypto Card</span>
          ...
          <div className="flex gap-4 justify-center pt-4">
            <Link
              href="/cards"
              className="px-8 py-4 bg-lime-600 hover:bg-lime-700 text-white rounded-lg font-medium transition-colors"
            >
              Browse Cards
            </Link>
            <Link
              href="/compare"
              className="px-8 py-4 border border-gray-700 hover:border-gray-600 text-white rounded-lg font-medium transition-colors"
            >
              Compare Cards
            </Link>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            ...
            <h3 className="text-lg font-semibold text-white mb-2">Compare Cards</h3>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            ...
            <h3 className="text-lg font-semibold text-white mb-2">Track Rewards</h3>
            <p className="text-gray-400">
      ...
      {featuredCards && featuredCards.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 py-16">
          <div className="flex justify-between items-center mb-8">
            ...
            <Link href="/cards" className="text-lime-400 hover:text-lime-300 text-sm font-medium">
              View all cards →
            </Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {featuredCards.map((card) => (
              <Link key={card.id} href={`/cards/${card.id}`}>
                <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden hover:border-lime-500/50 transition-all duration-300">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex justify-between mt-3 text-sm">
                      ...
                      <span className="text-lime-400 font-medium">{card.rewards_rate?.default}% back</span>
                    </div>
                  </div>
                </div>
              </Link>
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-6">
              <Link href="/cards" className="text-sm text-gray-500 hover:text-white transition-colors">
                Cards
              </Link>
              <Link href="/compare" className="text-sm text-gray-500 hover:text-white transition-colors">
                Compare
              </Link>
            </div>
          </div>
